...
void detKeypointsShiTomasi(vector<cv::KeyPoint> &keypoints, cv::Mat &img, bool bVis)
{
    ...
    int maxCorners = 50;//http://www.adept.net.au/news/newsletter/201211-nov/article_3D_stereo.shtmlg.rows * img.cols / max(1.0, minDistance); // max. num. of keypoints
...
{
    // compute detector parameters based on image size
    int blockSize = 4;     // for every pixel, a blockSize x blockSize neighbourhood is considered
    int apertureSize = 3;  // aperture for Sobel operator (must be odd)
    ...
    // Locate local maxima in the Harris response matrix and perform
    // a non-maxima suppression (NMS) in a local neighbourhood around
    // each maximum. The resulting coordinates shall be stored in a
    // list of keypoints of the type vector<cv::Keypoints>

    // Look for prominent corners  and instantiate keypoints
    double maxOverlap = 0.0; // max. permissible overlap between two
    // features in %, used during non-maxima suppression
                ...
                for (auto it = keypoints.begin(); it != keypoints.end(); ++it)
                {
                    ...
                    if (kptOverlap > maxOverlap)
                    {
                        ...
                        if (newKeyPoint.response > (*it).response)
                        {
                            ...
                            *it = newKeyPoint; // replace old keypoint with new one
